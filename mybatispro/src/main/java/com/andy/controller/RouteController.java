...
public class RouteController {
    ...
    public String login1(UserDto user,HttpServletRequest request,Model model) throws Exception{   
    	UserDto user1 = user;//userService.getUserByName(user.getName());   
        if(user1 == null) {   
            model.addAttribute("message", "用户不存在");   
            return "login";   
...
//        model.addAttribute("deptList", deptList);   
    ...
    public String doAdd(UserDto user,Model model) throws Exception{   
        try {   
    ...
    public String doUpdate(@PathVariable("id") int id, UserDto user,Model model) throws Exception{   
        try {   
...
//	 }  
	...
	public void getUser(HttpServletRequest request, HttpServletResponse response, Model model) {  
	    ...
	    UserDto user =null;// userService.getUserById(id);
